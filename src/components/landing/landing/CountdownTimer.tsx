...
export default function CountdownTimer() {
  ...
  const TimeBlock = ({ value, label }: { value: number; label: string }) => (
    <div className="flex flex-col items-center">
      <span className="font-syne text-lg font-bold text-white tabular-nums">
        {String(value).padStart(2, "0")}
      </span>
      <span className="text-[9px] text-white/70 uppercase tracking-wider">{label}</span>
    </div>
  );
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <div className="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-2xl px-4 py-3 shadow-2xl border border-white/20 backdrop-blur-sm">
        ...
        <p className="text-[10px] text-white/60 text-center mt-1">Beta Launch</p>
      </div>
    </div>
  );
}
